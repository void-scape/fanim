...
pub fn spawn(mut commands: Commands, renderer: Single<&Renderer>, ssaa: Single<&SsaaPipeline>) {
    commands.spawn(BulbPipeline::new(&renderer.device, ssaa.into_inner()));
}

pub fn compute_pass(
    renderer: Single<&Renderer>,
    pipeline: Single<&BulbPipeline>,
    ssaa: Single<&SsaaPipeline>,
    params: Single<(&Params, &Palette, &Bulb), With<EncodingTarget>>,
) {
    // TODO: only write these if they change, but a simple Ref check won't do
    let (params, palette, bulb) = params.into_inner();
    if **bulb <= 0.0 {
        return;
    }
    ...
    pub fn new(device: &wgpu::Device, ssaa: &SsaaPipeline) -> Self {
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(ssaa.bulb_render_target()),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
